...
export function Pagination({ meta, onPage }: PaginationProps) {
  ...
  return (
    <nav className="flex items-center justify-between gap-2 border-t border-line px-1 py-2 text-sm" aria-label={t('paginacion.aria')}>
      <span className="text-muted">{t('paginacion.registros', { n: totalElements })}</span>
      <div className="flex items-center gap-1">
        ...
        {numeros[numeros.length - 1] < totalPages - 1 && <span className="px-1 text-muted">…</span>}
        <button
          type="button"
