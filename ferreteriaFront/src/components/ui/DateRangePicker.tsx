...
import { construirPreset, PRESETS_RANGO, rangoFechas, type RangoFechas } from '@/lib/rango'
...
export function DateRangePicker({ valor, onChange }: DateRangePickerProps) {
  ...
  const presetPropio = useMemo(() => {
    ...
    if (valor.inicio === aLocalDate(ayer) && valor.fin === aLocalDate(ayer)) return 'ayer'
    ...
    <div className="flex flex-wrap items-end gap-2" data-testid="rango-fechas">
      ...
      {PRESETS_RANGO.map((p) => (
        <button
          ...
          className={`h-9 rounded-md border px-2.5 text-xs font-medium transition-colors ${
            presetPropio === p.id ? 'border-primary bg-primary text-white' : 'border-line bg-surface text-muted hover:bg-warmbg'
          }`}
        >
          {t(`rango.${p.id}`)}
        </button>
      ...
      <label className="flex items-center gap-1 text-xs text-muted">
        ...
        <input
          ...
          max={valor.fin}
          onChange={(e) => {
            if (e.target.value) onChange(rangoFechas(e.target.value, valor.fin))
          }}
      ...
      <label className="flex items-center gap-1 text-xs text-muted">
        ...
        <input
          ...
          max={hoyLocal()}
          onChange={(e) => {
            if (e.target.value) onChange(rangoFechas(valor.inicio, e.target.value))
          }}
          className="h-9 rounded-md border border-line bg-surface text-sm text-ink"
        />
      </label>
