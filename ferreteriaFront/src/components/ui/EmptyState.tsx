...
export function EmptyState({ title, descripcion, action }: { title?: string; descripcion?: string; action?: ReactNode }) {
  const t = useT()
  return (
    <div className="flex flex-col items-center justify-center gap-2 py-12 text-center text-muted">
      ...
      <p className="text-sm font-medium text-ink">{title ?? t('comun.sinResultados')}</p>
