...
export async function apiProductos(p: {
  ...
  const { data } = await http.get<PageEnvelope<Producto>>("/productos", {
    params,
  });
  return data;
}
...
export async function apiUnidadesMedida(): Promise<UnidadMedida[]> {
  const { data } = await http.get<PageEnvelope<UnidadMedida>>(
    "/unidades-medida",
...
export async function apiClientes(p: {
  q?: string;
  page: number;
  size: number;
  sort?: string;
  almacenId?: number;
}): Promise<PageEnvelope<Cliente>> {
  const params: Record<string, string | number> = {
    page: p.page,
    size: p.size,
  };
  ...
  if (p.sort) params.sort = p.sort;
  if (p.almacenId) params.almacenId = p.almacenId;
  const { data } = await http.get<PageEnvelope<Cliente>>("/clientes", {
    params,
  });
  return data;
}
...
export async function apiActualizarAlmacen(
  id: number,
  body: AlmacenRequest,
): Promise<Almacen> {
...
export async function apiActualizarEstadoAlmacen(
  id: number,
  activo: boolean,
): Promise<Almacen> {
  const { data } = await http.put<Envelope<Almacen>>(
    `/almacenes/${id}/estado`,
    { activo },
  );
  return data.data;
...
export async function apiProveedores(q?: string): Promise<Proveedor[]> {
  ...
  if (q) params.q = q;
  const { data } = await http.get<PageEnvelope<Proveedor>>("/proveedores", {
    params,
  });
  return data.data;
}

export async function apiProveedoresPaginado(p: {
  q?: string;
  page: number;
  size: number;
}): Promise<PageEnvelope<Proveedor>> {
  const params: Record<string, string | number> = {
    page: p.page,
    size: p.size,
  };
...
export async function apiCrearProveedor(
...
export async function apiActualizarProveedor(
  id: number,
  body: ProveedorRequest,
): Promise<Proveedor> {
  const { data } = await http.put<Envelope<Proveedor>>(
    `/proveedores/${id}`,
    body,
  );
  return data.data;
