...
import type { Envelope, PageEnvelope, Promocion, PromocionRequest } from './types'
...
export async function apiPromociones(f: PromocionesFiltros): Promise<PageEnvelope<Promocion>> {
  ...
  const { data } = await http.get<PageEnvelope<Promocion>>('/promociones', { params })
  return data
}
...
export async function apiCrearPromocion(body: PromocionRequest): Promise<Promocion> {
  const { data } = await http.post<Envelope<Promocion>>('/promociones', body)
  return data.data
}

export async function apiActualizarPromocion(id: number, body: PromocionRequest): Promise<Promocion> {
  const { data } = await http.put<Envelope<Promocion>>(`/promociones/${id}`, body)
  return data.data
}
