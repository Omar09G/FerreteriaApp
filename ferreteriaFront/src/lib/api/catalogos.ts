...
export async function apiCatalogoDatos(p: {
	...
	if (!ep)
		return {
			success: true,
			data: [],
			meta: { totalElements: 0, totalPages: 0, page: 0, size: p.size },
		};
	...
	const { data } = await http.get<PageEnvelope<Record<string, unknown>>>(
		ep.path,
		{ params },
	);
	return {
...
export async function apiCatalogoOpciones(
	clave: string,
	campo: string,
): Promise<Record<string, unknown>[]> {
	const { data } = await http.get<Envelope<Record<string, unknown>[]>>(
		`/catalogos/${clave}/opciones`,
		{
...
export async function apiCatalogoCrear(
	clave: string,
...
export async function apiCatalogoActualizar(
	clave: string,
	id: string | number,
	cuerpo: Record<string, unknown>,
): Promise<void> {
...
export async function apiCatalogoEliminar(
	clave: string,
	id: string | number,
): Promise<void> {
	const ep = catalogoEndpoint(clave);
