...
export async function apiAuditoria(
	f: AuditoriaFiltros,
): Promise<PageEnvelope<Auditoria>> {
	...
	const { data } = await http.get<PageEnvelope<Auditoria>>("/auditoria", {
		params,
	});
	return data;
}
...
export async function apiTablasAuditoria(): Promise<AuditoriaTabla[]> {
	const { data } =
		await http.get<Envelope<AuditoriaTabla[]>>("/auditoria/tablas");
	return data.data;
}
