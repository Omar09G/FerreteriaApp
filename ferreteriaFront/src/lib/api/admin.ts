...
export async function apiEmpleados(
	page: number,
	size = 15,
): Promise<PageEnvelope<Empleado>> {
	const { data } = await http.get<PageEnvelope<Empleado>>("/empleados", {
		params: { page, size },
	});
	return data;
}

export async function apiCrearEmpleado(
...
export async function apiUsuarios(
	page: number,
	size = 15,
): Promise<PageEnvelope<Usuario>> {
	const { data } = await http.get<PageEnvelope<Usuario>>("/usuarios", {
		params: { page, size },
	});
	return data;
}

export async function apiCrearUsuario(
...
export async function apiSetRolesUsuario(
	id: number,
	roles: string[],
): Promise<Usuario> {
	const { data } = await http.put<Envelope<Usuario>>(`/usuarios/${id}/roles`, {
		roles,
	});
	return data.data;
}
...
export async function apiResetPassword(
	...
	const { data } = await http.patch<Envelope<Usuario>>(
...
export async function apiRoles(): Promise<Rol[]> {
	const { data } = await http.get<PageEnvelope<Rol>>("/roles", {
		params: { page: 0, size: 100 },
	});
	return data.data;
}

export async function apiRolesPaginado(p: {
	page: number;
	size: number;
}): Promise<PageEnvelope<Rol>> {
	const { data } = await http.get<PageEnvelope<Rol>>("/roles", {
...
export async function apiActualizarRol(
	id: number,
	body: RolRequest,
): Promise<Rol> {
...
export async function apiPermisos(): Promise<Permiso[]> {
	const { data } = await http.get<PageEnvelope<Permiso>>("/permisos", {
...
export async function apiSetPermisosRol(
	id: number,
	permisos: string[],
): Promise<string[]> {
	const { data } = await http.put<Envelope<string[]>>(`/roles/${id}/permisos`, {
		permisos,
	});
	return data.data;
}

export async function apiNomina(p: {
	estado?: string;
	desde?: string;
	hasta?: string;
	page: number;
	size: number;
}): Promise<PageEnvelope<Nomina>> {
