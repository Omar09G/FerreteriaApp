...
export async function apiCancelarVenta(
	id: number,
	body: VentaCancelRequest,
): Promise<Venta> {
...
export async function apiCotizaciones(p: {
	...
	page: number;
	size: number;
}): Promise<PageEnvelope<Cotizacion>> {
	const params: Record<string, string | number> = {
		page: p.page,
		size: p.size,
	};
	...
	if (p.desde) params.desde = p.desde;
	if (p.hasta) params.hasta = p.hasta;
	const { data } = await http.get<PageEnvelope<Cotizacion>>("/cotizaciones", {
		params,
	});
	return data;
}

export async function apiCrearCotizacion(
...
export async function apiConvertirCotizacion(
	id: number,
	almacenId: number,
	formaPagoId: number,
	cajaId?: number,
): Promise<Cotizacion> {
	const { data } = await http.post<Envelope<Cotizacion>>(
		`/cotizaciones/${id}/convertir`,
		null,
		{ params: { almacenId, formaPagoId, cajaId } },
	);
	return data.data;
}

export async function apiDevolucionesDeVenta(
	ventaId: number,
): Promise<Devolucion[]> {
	const { data } = await http.get<Envelope<Devolucion[]>>(
		`/devoluciones/venta/${ventaId}`,
	);
	return data.data;
}

export async function apiCrearDevolucion(
...
export async function apiRentas(p: {
	estado?: string;
	desde?: string;
	hasta?: string;
	page: number;
	size: number;
}): Promise<PageEnvelope<Renta>> {
	const params: Record<string, string | number> = {
		page: p.page,
		size: p.size,
	};
...
export async function apiDevolucionRenta(
	id: number,
	body: RentaDevolucionRequest,
): Promise<Renta> {
	const { data } = await http.post<Envelope<Renta>>(
		`/rentas/${id}/devolucion`,
		body,
	);
	return data.data;
}
