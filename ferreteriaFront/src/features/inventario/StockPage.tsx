...
import { AlertTriangle, ArrowDownCircle, ArrowUpCircle, Sliders, Warehouse } from 'lucide-react'
...
import { MOTIVOS_MOVIMIENTO, type Inventario, type MovimientoInventarioRequest } from '@/lib/api/types'
...
function AjusteDialog({ target, onClose }: { target: AjusteTarget | null; onClose: () => void }) {
  ...
  const nuevoStock =
    target ? (tipo === 'ENTRADA' ? target.stockActual + cantidadNum : target.stockActual - cantidadNum) : 0
  const invalido =
    !target ||
    cantidad === '' ||
  ...
  const ajuste = useMutation({
    ...
    onSuccess: (m) => {
      success(`${tipo === 'ENTRADA' ? 'Entrada' : 'Salida'} registrada: ${formatoNumero(m.cantidad)} uds.`)
      queryClient.invalidateQueries({ queryKey: ['stock'] })
      queryClient.invalidateQueries({ queryKey: ['movimientos'] })
      queryClient.invalidateQueries({ queryKey: ['dashboard'] })
      onClose()
    },
    onError: (err) => error(esApiError(err) ? err.mensajeParaUsuario() : String(err)),
  })
  ...
  return (
    <Dialog open={target !== null} onClose={() => !ajuste.isPending && onClose()} title="Ajustar existencia" width="max-w-md">
      {target && (
        <div className="space-y-3">
          <div className="rounded-md bg-canvas p-3 text-sm">
            ...
            <p className="mt-1 text-xs text-muted">
              Existencia actual:{' '}
              <span className="font-mono font-semibold text-ink">{formatoNumero(target.stockActual)}</span> uds.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <span className="mb-1 block text-sm font-medium text-ink">Tipo de movimiento</span>
              <div className="grid grid-cols-2 gap-1 rounded-md border border-line bg-surface p-1">
                <button
                  ...
                  className={`flex items-center justify-center gap-1 rounded px-2 py-1.5 text-xs font-medium transition-colors ${
                    tipo === 'ENTRADA' ? 'bg-green-600 text-white' : 'text-muted hover:bg-warmbg'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex items-center justify-center gap-1 rounded px-2 py-1.5 text-xs font-medium transition-colors ${
                    tipo === 'SALIDA' ? 'bg-red-600 text-white' : 'text-muted hover:bg-warmbg'
                  }`}
                >
          ...
          <div className="rounded-md border border-line p-3 text-sm">
            <div className="flex items-center justify-between">
              ...
              >
                {Number.isNaN(nuevoStock) ? '—' : formatoNumero(nuevoStock)} uds.
              </span>
            </div>
            {intento && invalido && (
          ...
          <div className="flex justify-end gap-2 pt-1">
            <Button variant="ghost" disabled={ajuste.isPending} onClick={onClose}>
              Cancelar
            </Button>
            <Button disabled={invalido || ajuste.isPending} onClick={guardar}>
              {tipo === 'ENTRADA' ? <ArrowUpCircle className="h-4 w-4" /> : <ArrowDownCircle className="h-4 w-4" />}
              {ajuste.isPending ? 'Registrando…' : `Registrar ${tipo === 'ENTRADA' ? 'entrada' : 'salida'}`}
            </Button>
          </div>
        </div>
      )}
...
export default function StockPage() {
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted">
            Nivel de stock por almacén y productos en riesgo de desabasto. Usa <span className="font-medium">Ajustar</span> para registrar entradas o salidas manuales.
          </p>
        </div>
        <Button
          variant="secondary"
      ...
      <AjusteDialog target={ajusteTarget} onClose={() => setAjusteTarget(null)} />
    </div>
  )
}
