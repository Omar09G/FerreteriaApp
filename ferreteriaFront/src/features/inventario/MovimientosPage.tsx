...
export default function MovimientosPage() {
  ...
  const [filtro, setFiltro] = useState<{ productoId?: number; almacenId?: number }>({})
  ...
  const { data, isLoading, error, isFetching } = useQuery({
    queryKey: ['movimientos', rango.inicio, rango.fin, filtro.productoId, filtro.almacenId, page],
    queryFn: () =>
      apiMovimientos({
        inicio: rango.inicio,
        fin: rango.fin,
  ...
  const columnas: Columna<MovimientoInventario>[] = [
    { key: 'f', header: 'Fecha', render: (v) => <span className="whitespace-nowrap">{formatoFechaHora(v.creadoEn)}</span> },
    ...
    { key: 't', header: 'Tipo', render: (v) => (v.tipo === 'ENTRADA' ? <Badge tone="success">Entrada</Badge> : <Badge tone="danger">Salida</Badge>) },
    { key: 'c', header: 'Cantidad', align: 'right', render: (v) => formatoNumero(v.cantidad) },
    { key: 'cu', header: 'Costo unit.', align: 'right', render: (v) => formatoMoneda(v.costoUnitario) },
    { key: 'm', header: 'Motivo', render: (v) => v.motivoNombre ?? '—' },
    { key: 'r', header: 'Referencia', render: (v) => (v.refTabla ? `${v.refTabla}#${v.refId}` : '—') },
  ]
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-ink">Movimientos de inventario</h1>
          <p className="text-sm text-muted">Bitácora de entradas y salidas. Por defecto se muestran las de hoy.</p>
        </div>
      ...
      <Card>
        <div className="flex flex-wrap items-end gap-2">
          <Input label="Producto (id)" value={productoId} onChange={(e) => setProductoId(e.target.value)} placeholder="Ej. 12" className="w-40" inputMode="numeric" />
          <Input label="Almacén (id)" value={almacenId} onChange={(e) => setAlmacenId(e.target.value)} placeholder="Ej. 1" className="w-40" inputMode="numeric" />
      ...
      {data && (
        <Card titulo={`Movimientos (${data.meta.totalElements})`}>
          <DataTable columnas={columnas} items={data.data} rowKey={(v) => v.movimientoId} loading={isFetching} />
