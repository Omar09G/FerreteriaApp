...
function RentaForm({
	...
	guardando: boolean;
	onGuardar: (body: RentaRequest) => void;
	onClose: () => void;
}) {
	const clientes = useQuery({
		queryKey: ["clientes-renta"],
		queryFn: () => apiClientes({ page: 0, size: 50 }),
	});
	const almacenes = useQuery({
		queryKey: ["almacenes-renta"],
	...
	const totalPartidas = partidas.reduce(
		(acc, x) => acc + x.cantidad * x.costoDia,
		0,
	);
	...
	const invalido =
		...
		fechaDevEsperada === "" ||
		partidas.length === 0 ||
		partidas.some((x) => x.cantidad <= 0 || x.costoDia < 0) ||
		depositoNum < 0;
		...
		<form onSubmit={enviar} className="space-y-3" noValidate>
			<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
				...
				<Input
					...
					type="date"
					required
					value={fechaDevEsperada}
					onChange={(e) => setFechaDevEsperada(e.target.value)}
				/>
				<Input
					label="Depósito"
					type="number"
			...
			{cajaConTurno && (
				<p className="text-xs text-emerald-600">
					Turno abierto: el depósito se registrará a esa caja.
				</p>
			)}
			{cajaSinTurno && (
			...
			{partidas.length > 0 && (
				<div className="space-y-1.5">
					<div
						aria-hidden
						className="flex items-center gap-2 px-2 text-[11px] font-medium uppercase tracking-wide text-muted"
					>
					...
					{partidas.map((x) => (
						...
						>
							...
							<input
								...
								onChange={(e) =>
									setPartidas((prev) =>
										prev.map((y) =>
											y.productoId === x.productoId
												? { ...y, costoDia: Number(e.target.value) }
												: y,
										),
									)
								}
							...
							<span className="w-24 shrink-0 text-right text-sm font-semibold tabular-nums">
								{formatoMoneda(x.cantidad * x.costoDia)}
							</span>
						</div>
					))}
			...
			{intento && invalido && (
				<p className="text-xs text-red-600">
					Completa cliente, almacén, caja con turno abierto, fecha de devolución
					y al menos una partida.
				</p>
			)}
...
function DevolucionForm({
	...
	renta: Renta;
	guardando: boolean;
	onGuardar: (body: RentaDevolucionRequest) => void;
	onClose: () => void;
}) {
	const [dias, setDias] = useState<Record<number, number>>(() =>
		Object.fromEntries(renta.detalles.map((d) => [d.productoId, 1])),
	);
	const [intento, setIntento] = useState(false);
	...
	const enviar = (e: { preventDefault: () => void }) => {
		...
		if (invalido) return;
		onGuardar({
			detalles: renta.detalles.map((d) => ({
				productoId: d.productoId,
				diasCobrados: dias[d.productoId] ?? 0,
			})),
		});
	};
		...
		<form onSubmit={enviar} className="space-y-3" noValidate>
			<div className="rounded-md bg-canvas px-3 py-2 text-sm">
				...
				<p className="text-muted">
					Devolución esperada {formatoFecha(renta.fechaDevEsperada)} · depósito{" "}
					{formatoMoneda(renta.deposito)}
				</p>
			</div>
			<div className="space-y-1.5">
						...
						<label className="flex items-center gap-1 text-xs text-muted">
							...
							<input
								...
								onChange={(e) =>
									setDias((prev) => ({
										...prev,
										[d.productoId]: Number(e.target.value),
									}))
								}
			...
			{intento && invalido && (
				<p className="text-xs text-red-600">
					Los días cobrados no pueden ser negativos.
				</p>
			)}
			<div className="flex justify-end gap-2">
				<Button type="button" variant="ghost" onClick={onClose}>
...
export default function RentasPage() {
	...
	const devolver = useMutation({
		mutationFn: ({ id, body }: { id: number; body: RentaDevolucionRequest }) =>
			apiDevolucionRenta(id, body),
		onSuccess: () => {
	...
	return (
		<div className="space-y-4">
			<header className="flex flex-wrap items-center justify-between gap-3">
				<div>
					...
					<p className="text-sm text-muted">
						Renta de herramientas y su devolución con días cobrados.
					</p>
				</div>
			...
			<Dialog
				...
				onClose={() => !devolver.isPending && setDevolviendo(null)}
				title="Registrar devolución"
				width="max-w-lg"
			>
				{devolviendo && (
					<DevolucionForm
						renta={devolviendo}
						guardando={devolver.isPending}
						onGuardar={(body) =>
							devolver.mutate({ id: devolviendo.rentaId, body })
						}
			...
			<ConfirmDialog
				...
				onConfirm={() =>
					cancelarConfirmacion && cancelar.mutate(cancelarConfirmacion.rentaId)
				}
			>
