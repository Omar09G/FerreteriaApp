...
export default function MejoresClientesPage() {
	...
	const columnas: Columna<MejorCliente>[] = [
		...
		},
		{
			...
			header: "Compras",
			align: "right",
			exportar: (v) => formatoNumero(v.numCompras),
		...
		},
		{
			...
			align: "right",
			exportar: (v) => formatoMoneda(v.totalComprado),
		...
		<div className="space-y-4">
			...
			{data && data.length > 0 && (
				<Card
					...
					actions={
						<ExportarExcel columnas={columnas} items={data} archivo="mejores-clientes" />
					}
				>
					<DataTable
						columnas={columnas}
