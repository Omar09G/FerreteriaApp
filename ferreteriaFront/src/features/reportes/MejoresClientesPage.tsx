...
export default function MejoresClientesPage() {
  ...
  const columnas: Columna<MejorCliente>[] = [
    { key: 'r', header: 'Ranking', render: (v) => <span className="font-medium">{v.rankingMes}°</span> },
    { key: 'n', header: 'Cliente', render: (v) => v.cliente },
    { key: 'c', header: 'Compras', align: 'right', render: (v) => formatoNumero(v.numCompras) },
    { key: 't', header: 'Total comprado', align: 'right', render: (v) => formatoMoneda(v.totalComprado) },
    { key: 'p', header: 'Ticket promedio', align: 'right', render: (v) => formatoMoneda(v.ticketPromedio) },
    { key: 'h', header: 'Ranking histórico', align: 'right', render: (v) => v.rankingHistorico },
  ]
    ...
    <div className="space-y-4">
      ...
      {data && data.length > 0 && (
        <Card titulo="Ranking del periodo">
          <DataTable columnas={columnas} items={data} rowKey={(v) => v.clienteId} caption="Mejores clientes" />
        </Card>
      )}
      {data && data.length === 0 && !isLoading && (
        <EmptyState title="Sin clientes en el periodo" descripcion="Cambia el rango de fechas." />
      )}
    </div>
  )
}