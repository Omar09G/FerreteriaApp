...
export default function MejoresVendedoresPage() {
  ...
  const columnas: Columna<MejorVendedor>[] = [
    ...
    { key: 'c', header: 'Ventas', align: 'right', render: (v) => formatoNumero(v.numVentas) },
    { key: 't', header: 'Total vendido', align: 'right', render: (v) => formatoMoneda(v.totalVendido) },
    { key: 'p', header: 'Ticket promedio', align: 'right', render: (v) => formatoMoneda(v.ticketPromedio) },
    { key: 'u', header: 'Utilidad', align: 'right', render: (v) => formatoMoneda(v.utilidadGenerada) },
  ]
    ...
    <div className="space-y-4">
      ...
      {data && data.length > 0 && (
        <Card titulo="Ranking del periodo">
          <DataTable columnas={columnas} items={data} rowKey={(v) => v.usuarioId} caption="Mejores vendedores" />
        </Card>
      )}
