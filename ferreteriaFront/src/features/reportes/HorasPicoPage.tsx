...
export default function HorasPicoPage() {
  ...
  const columnas: Columna<NonNullable<typeof data>[number]>[] = [
    { key: 'h', header: 'Hora', render: (v) => `${String(v.hora).padStart(2, '0')}:00` },
    { key: 'n', header: 'Ventas', align: 'right', render: (v) => formatoNumero(v.numVentas) },
    { key: 't', header: 'Total acumulado', align: 'right', render: (v) => formatoMoneda(v.totalAcumulado) },
    { key: 'p', header: 'Ticket promedio', align: 'right', render: (v) => formatoMoneda(v.ticketPromedio) },
  ]
    ...
    <div className="space-y-4">
      ...
      {data && data.length > 0 && (
        <>
          <Card titulo="Número de ventas por hora">
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={data} margin={{ left: 8, right: 8 }}>
                ...
                <XAxis dataKey="hora" tickFormatter={(h: number) => `${h}:00`} stroke="#57534e" fontSize={12} />
                <YAxis stroke="#57534e" fontSize={12} />
                <Tooltip
                  formatter={(value, name) => [formatoNumero(Number(value)), String(name)]}
          ...
          <Card titulo="Detalle">
            <DataTable columnas={columnas} items={data} rowKey={(v) => v.hora} caption="Ventas por hora" />
          </Card>
        </>
      )}
      {data && data.length === 0 && !isLoading && <EmptyState title="Sin ventas en el periodo" descripcion="Cambia el rango de fechas para ver más información." />}
    </div>
  )
}