...
const TIPOS: { value: TipoPromocion; label: string; ayuda: string }[] = [
  { value: 'DESCUENTO_PRODUCTO', label: 'Descuento por producto', ayuda: 'Aplica valor_pct o valor_monto al producto' },
  { value: 'DESCUENTO_TOTAL_VENTA', label: 'Descuento al total', ayuda: 'Aplica valor_pct o valor_monto al subtotal' },
  { value: 'POR_CANTIDAD', label: 'Por cantidad mínima', ayuda: 'Requiere compra_min_cantidad + valor' },
  { value: 'NXM', label: 'Lleva N paga M', ayuda: 'Define lleva y paga' },
  { value: 'PRECIO_ESPECIAL', label: 'Precio especial', ayuda: 'Precio fijo independiente' },
]

const ESTADOS: EstadoPromocion[] = ['ACTIVA', 'PROGRAMADA', 'FINALIZADA', 'CANCELADA']
...
const estadoTone = (e: EstadoPromocion): 'success' | 'warning' | 'danger' | 'default' => {
...
function PromocionForm({
  ...
  const productos = useQuery({
    ...
    queryFn: () =>
      apiProductos({ q: busquedaProducto || undefined, page: 0, size: 50, sort: 'nombre' }),
  })

  const categoriasList = useMemo(() => (categorias.data ? aplanarCategorias(categorias.data) : []), [categorias.data])

  const setField = <K extends keyof PromocionRequest>(k: K, v: PromocionRequest[K]) =>
    setBody((b) => ({ ...b, [k]: v }))
  ...
  const toggleCategoria = (id: number) =>
    setBody((b) => ({
      ...b,
      categorias: b.categorias.includes(id) ? b.categorias.filter((x) => x !== id) : [...b.categorias, id],
    }))

  const agregarProducto = (id: number) =>
    setBody((b) => ({ ...b, productos: b.productos.includes(id) ? b.productos : [...b.productos, id] }))
  ...
  const productosSeleccionados = productos.data?.data.filter((p) => body.productos.includes(p.productoId)) ?? []

  const esDescuento = body.tipo === 'DESCUENTO_PRODUCTO' || body.tipo === 'DESCUENTO_TOTAL_VENTA' || body.tipo === 'POR_CANTIDAD'
  ...
  return (
    <form onSubmit={enviar} className="grid grid-cols-1 gap-3 sm:grid-cols-2" noValidate>
      <Input
        ...
        error={intento && body.nombre.trim() === '' ? t('comun.obligatorio') : undefined}
      />
      <Select
      ...
      {esDescuento && (
        <>
          <Input
            ...
            step="0.01"
            value={body.valorPct ?? ''}
            onChange={(e) => setField('valorPct', e.target.value === '' ? undefined : Number(e.target.value))}
          />
          <Input
            ...
            onChange={(e) => setField('valorMonto', e.target.value === '' ? undefined : Number(e.target.value))}
          />
          <Input
            ...
            onChange={(e) => setField('compraMinTotal', e.target.value === '' ? undefined : Number(e.target.value))}
          />
          <Input
            ...
            onChange={(e) => setField('compraMinCantidad', e.target.value === '' ? undefined : Number(e.target.value))}
          />
        </>
      )}
      ...
      {esNxm && (
        <>
          <Input
            ...
            required
            value={body.lleva ?? ''}
            onChange={(e) => setField('lleva', e.target.value === '' ? undefined : Number(e.target.value))}
          />
          <Input
            ...
            required
            value={body.paga ?? ''}
            onChange={(e) => setField('paga', e.target.value === '' ? undefined : Number(e.target.value))}
          />
        </>
      )}
      ...
      {esPrecio && (
        <Input
          ...
          onChange={(e) => setField('precioEspecial', e.target.value === '' ? undefined : Number(e.target.value))}
        />
      )}
      ...
      <Input
        ...
        onChange={(e) => setField('maxUsosTotal', e.target.value === '' ? undefined : Number(e.target.value))}
      />
      <Input
        label={t('catalogo.promociones.campos.maxUsosCliente')}
        type="number"
        value={body.maxUsosCliente ?? ''}
        onChange={(e) => setField('maxUsosCliente', e.target.value === '' ? undefined : Number(e.target.value))}
      />
      ...
      <div className="sm:col-span-2">
        <span className="text-xs font-medium text-muted">{t('catalogo.promociones.campos.dias')}</span>
        <div className="mt-1 flex flex-wrap gap-2">
          {DIAS.map((d) => (
            <label
              key={d.d}
              className={`flex h-9 w-9 cursor-pointer items-center justify-center rounded-md border text-xs font-semibold ${
                body.diasSemana.includes(d.d) ? 'border-primary bg-primary text-white' : 'border-line'
              }`}
            >
              <input
                type="checkbox"
          ...
          {intento && body.diasSemana.length === 0 && (
            <span className="ml-2 text-xs text-red-600">{t('catalogo.promociones.campos.seleccionaUnDia')}</span>
          )}
        </div>
      </div>
      ...
      <label className="flex items-center gap-2 sm:col-span-2">
        ...
        />
        <span className="text-sm">{t('catalogo.promociones.campos.soloMayoristas')}</span>
      </label>
      ...
      {esEdicion && (
        <div className="flex items-end text-sm text-muted">{t('catalogo.promociones.campos.usosActuales', { n: inicial!.usosActual })}</div>
      )}

      <div className="sm:col-span-2 mt-2">
        <span className="text-xs font-medium text-muted">{t('catalogo.promociones.campos.categoriasAplicables')}</span>
        {categorias.isLoading ? (
          ...
          <div className="mt-1 grid max-h-40 grid-cols-2 gap-1 overflow-y-auto rounded-md border border-line p-2 sm:grid-cols-3">
            {categoriasList.map((c) => (
              <label key={c.categoriaId} className="flex items-center gap-1.5 text-xs">
                <input
                  type="checkbox"
                  ...
                  className="accent-primary"
                />
                <span style={{ paddingLeft: `${(c.nivel ?? 0) * 8}px` }}>{c.nombre}</span>
              </label>
            ))}
          </div>
        )}
      ...
      <div className="sm:col-span-2 mt-2">
        <span className="text-xs font-medium text-muted">{t('catalogo.promociones.campos.productosAplicables')}</span>
        <Input
          className="mt-1"
          ...
          <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
            <div className="max-h-40 overflow-y-auto rounded-md border border-line p-2">
              {productos.data?.data.length === 0 && (
                <span className="text-xs text-muted">{t('catalogo.promociones.campos.sinCoincidencias')}</span>
              )}
              {productos.data?.data
                ...
                .map((p) => (
                  ...
                  >
                    <span className="font-mono text-muted">{p.codigo}</span> · {p.nombre}
                  </button>
                ))}
            </div>
            <div className="rounded-md border border-line p-2">
              <div className="mb-1 text-xs font-medium">{t('catalogo.promociones.campos.seleccionados')}</div>
              {productosSeleccionados.length === 0 && (
                <span className="text-xs text-muted">{t('catalogo.promociones.campos.ninguno')}</span>
              )}
              {productosSeleccionados.map((p) => (
                ...
                >
                  <span className="font-mono text-muted">{p.codigo}</span> · {p.nombre}
                  <span className="ml-2 text-red-600">{t('catalogo.promociones.campos.quitar')}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="sm:col-span-2 mt-2 flex justify-end gap-2">
        <Button variant="ghost" type="button" disabled={guardando} onClick={cerrar}>
          {t('comun.cancelar')}
        </Button>
...
export default function PromocionesPage() {
  ...
  const [filtros, setFiltros] = useState<PromocionesFiltros>({ page: 0, size: 15 })
  ...
  const actualizar = useMutation({
    mutationFn: ({ id, body }: { id: number; body: PromocionRequest }) => apiActualizarPromocion(id, body),
    onSuccess: () => {
  ...
  const columnas: Columna<Promocion>[] = useMemo(
    () => [
      { key: 'nombre', header: t('comun.nombre'), render: (v) => v.nombre },
      {
        ...
        header: t('comun.tipo'),
        render: (v) => (
          <Badge tone="default">{t(`catalogo.promociones.tipos.${v.tipo}` as never)}</Badge>
        ),
      },
      {
        key: 'valor',
        ...
        render: (v) => {
          if (v.tipo === 'PRECIO_ESPECIAL') return `$${(v.precioEspecial ?? 0).toFixed(2)}`
          if (v.tipo === 'NXM')
            return t('catalogo.promociones.campos.llevaPagaLabel', { lleva: v.lleva ?? 0, paga: v.paga ?? 0 })
      ...
      },
      {
        ...
        render: (v) =>
          v.maxUsosTotal
            ? t('catalogo.promociones.campos.usosDe', { actual: v.usosActual, max: v.maxUsosTotal })
            : `${v.usosActual}`,
      },
      {
        key: 'acc',
        ...
        render: (v) =>
          puedeAdministrar ? (
            <div className="flex justify-end gap-1">
              <button
                type="button"
                aria-label={t('catalogo.promociones.aria.editar', { nombre: v.nombre })}
              ...
              <button
                type="button"
                aria-label={t('catalogo.promociones.aria.eliminar', { nombre: v.nombre })}
  ...
  const inicialForm: Promocion | undefined = esDialogoEdicion ? detalle.data : undefined
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-semibold text-ink">
            <Percent className="h-6 w-6 text-primary" /> {t('catalogo.promociones.titulo')}
          </h1>
          <p className="text-sm text-muted">{t('catalogo.promociones.subtitulo')}</p>
        </div>
        {puedeAdministrar && (
      ...
      <Card>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          <Input
            ...
            onChange={(e) => setFiltros((f) => ({ ...f, nombre: e.target.value, page: 0 }))}
          />
          <Select
            label={t('catalogo.promociones.filtros.tipo')}
            value={filtros.tipo ?? ''}
            onChange={(e) => setFiltros((f) => ({ ...f, tipo: e.target.value || undefined, page: 0 }))}
          >
          ...
          <Select
            ...
            onChange={(e) => setFiltros((f) => ({ ...f, estado: e.target.value || undefined, page: 0 }))}
          >
            <option value="">{t('catalogo.promociones.filtros.todos')}</option>
            {ESTADOS.map((e) => (
              <option key={e} value={e}>
      ...
      {lista.data && <Pagination meta={lista.data.meta} onPage={(p) => setFiltros((f) => ({ ...f, page: p }))} />}
      ...
      <Dialog
        ...
        title={esDialogoEdicion ? t('catalogo.promociones.editar') : t('catalogo.promociones.nueva')}
        width="max-w-3xl"
      >
      ...
      <ConfirmDialog
        ...
        onConfirm={() => eliminarPromo && eliminar.mutate(eliminarPromo.promocionId)}
      >
