  ...
  return (
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-muted">Administración de catálogos maestros. Solo administradores pueden modificar.</p>
      </div>
          ...
          {paneles.map((p) => (
            ...
            >
              <div className="flex items-center gap-3">
                <span className="rounded-md bg-orange-100 p-2 text-primary">{ICONOS[p.clave] ?? <Boxes className="h-5 w-5" />}</span>
                <div className="min-w-0">
                  <p className="truncate font-medium text-ink group-hover:text-primary">{p.nombre}</p>
