...
import type { CampoCatalogo, CatalogoDescriptor, FilaCatalogo } from '@/lib/api/types'
...
function aPayload(campos: CampoCatalogo[], form: Record<string, string>, editar: boolean): Record<string, unknown> {
...
function CatalogoForm({
  ...
  const invalido = campos.some((c) => c.requerido && (form[c.nombre] ?? '').trim() === '')

  const set = (nombre: string, val: string) => setForm((f) => ({ ...f, [nombre]: val }))
  ...
  const renderControl = (c: CampoCatalogo) => {
    ...
    if (c.opcionesTabla) {
      ...
      return (
        <Select key={c.nombre} label={etiqueta} required={c.requerido} value={form[c.nombre] ?? ''} onChange={(e) => set(c.nombre, e.target.value)}>
          <option value="">— Seleccionar —</option>
          {opts.map((o, i) => {
            const valor = String(o.clave)
            const texto = c.opcionesColumnas?.map((col) => String(o[col] ?? '')).filter(Boolean).join(' · ') || valor
            return (
  ...
  return (
    <form onSubmit={enviar} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      {intento && invalido && <p className="col-span-full text-sm text-red-600">Completa los campos obligatorios.</p>}
...
export default function CatalogoCrudPage({ clave }: { clave: string }) {
  ...
  const { data: paneles } = useQuery({
    ...
    queryFn: async () => (await import('@/lib/api/catalogos')).apiCatalogosPaneles(),
    staleTime: Infinity,
  })
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: () => apiCatalogoDatos({ clave, q: buscado || undefined, page, size: TAMANIO }),
  ...
  const { data: camposOpciones } = useQuery({
    ...
    queryFn: async () => {
      const fkCampos = (descriptor?.campos ?? []).filter((c) => Boolean(c.opcionesTabla))
      const entradas = await Promise.all(
        fkCampos.map(async (c) => [c.nombre, await apiCatalogoOpciones(c.opcionesTabla!, c.nombre)] as const),
      )
  ...
  const mutationCrear = useMutation({
    mutationFn: (body: Record<string, unknown>) => apiCatalogoCrear(clave, body),
    onSuccess: () => {
  ...
  const mutationEditar = useMutation({
    mutationFn: (body: Record<string, unknown>) => apiCatalogoActualizar(clave, editing?.__pk ?? '', body),
    onSuccess: () => {
  ...
  const columnas = useMemo<Columna<FilaCatalogo>[]>(() => {
    ...
    const cols: Columna<FilaCatalogo>[] = descriptor.campos
      ...
      .map((c) => ({
        key: c.nombre,
        header: c.etiqueta,
        render: (item) => (c.tipo === 'BOOLEAN' ? (item[c.nombre] ? <Badge tone="success">Sí</Badge> : <Badge tone="default">No</Badge>) : textoCelda(item[c.nombre])),
      }))
    cols.push({
      key: '__acciones',
      header: 'Acciones',
      render: (item) => (
        <div className="flex gap-1">
          <Button variant="ghost" size="sm" onClick={() => { setEditing(item); setDialogoAbierto(true) }} aria-label="Editar">
            <Pencil className="h-4 w-4" />
          </Button>
          {descriptor.soportaBajaLogica && (
            <Button variant="ghost" size="sm" onClick={() => setAEliminar(item)} aria-label="Eliminar">
  ...
  return (
    <div className="space-y-4 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-ink">{descriptor.nombre}</h1>
          <p className="text-sm text-muted">
            Catálogo de administración · {descriptor.campos.filter((c) => !c.esActivo).length} campos ·{' '}
            {descriptor.tabla}
          </p>
        </div>
      ...
      <Dialog
        ...
        title={editing ? `Editar ${descriptor.nombre}` : `Nuevo ${descriptor.nombre}`}
        width="max-w-2xl"
      >
        <CatalogoForm
          ...
          onGuardar={(payload) => (editing ? mutationEditar.mutate(payload) : mutationCrear.mutate(payload))}
          onClose={() => {
            if (mutationCrear.isPending || mutationEditar.isPending) return
            setDialogoAbierto(false)
            setEditing(null)
      ...
      >
        ¿Deseas desactivar este registro? Se conserva en el historial pero dejará de estar disponible.
      </ConfirmDialog>
    </div>
  )
}
