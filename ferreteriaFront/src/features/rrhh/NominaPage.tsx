...
function NominaForm({
  ...
  return (
    <form onSubmit={enviar} className="grid grid-cols-1 gap-3 sm:grid-cols-2" noValidate>
      <Select label="Empleado" required value={empleadoId} onChange={(e) => setEmpleadoId(e.target.value ? Number(e.target.value) : '')} className="sm:col-span-2">
      ...
      <Input label="Periodo inicial" required type="date" value={periodoIni} onChange={(e) => setPeriodoIni(e.target.value)} />
      <Input label="Periodo final" required type="date" value={periodoFin} onChange={(e) => setPeriodoFin(e.target.value)} error={intento && periodoInvalido ? 'El periodo final debe ser igual o posterior al inicial.' : undefined} />
      <Input label="Días pagados" required type="number" inputMode="decimal" min="1" step="0.5" value={diasPagados} onChange={(e) => setDiasPagados(e.target.value)} />
      <Input label="Percepciones" required type="number" inputMode="decimal" min="0" step="0.01" value={percepciones} onChange={(e) => setPercepciones(e.target.value)} />
      <Input label="Deducciones" required type="number" inputMode="decimal" min="0" step="0.01" value={deducciones} onChange={(e) => setDeducciones(e.target.value)} />
      <Input label="Notas (opcional)" value={notas} onChange={(e) => setNotas(e.target.value)} className="sm:col-span-2" />
      {intento && invalido && <p className="text-xs text-red-600 sm:col-span-2">Completa los campos obligatorios y verifica los valores.</p>}
...
export default function NominaPage() {
  ...
  const invalidar = () => queryClient.invalidateQueries({ queryKey: ['nomina'] })
  ...
  const columnas: Columna<Nomina>[] = [
    { key: 'emp', header: 'Empleado', render: (v) => <span className="font-medium text-ink">{v.empleado}</span> },
    {
      key: 'per',
      header: 'Periodo',
      render: (v) => (
    ...
    { key: 'dias', header: 'Días', render: (v) => <span className="tabular-nums">{v.diasPagados}</span> },
    { key: 'percep', header: 'Percepciones', align: 'right', render: (v) => <span className="tabular-nums">{formatoMoneda(v.percepciones)}</span> },
    { key: 'ded', header: 'Deducciones', align: 'right', render: (v) => <span className="tabular-nums">{formatoMoneda(v.deducciones)}</span> },
    { key: 'neto', header: 'Neto a pagar', align: 'right', render: (v) => <span className="font-medium tabular-nums">{formatoMoneda(v.netoPagar)}</span> },
    { key: 'est', header: 'Estado', render: (v) => <EstadoBadge estado={v.estado} /> },
    { key: 'fpago', header: 'Fecha de pago', render: (v) => (v.fechaPago ? <span className="tabular-nums">{formatoFecha(v.fechaPago)}</span> : '—') },
    {
      key: 'acc',
      header: 'Acciones',
      align: 'right',
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={dialogoAbierto} onClose={() => !crear.isPending && setDialogoAbierto(false)} title="Nueva nómina" width="max-w-lg">
        <NominaForm
