...
import { apiAuditoria, apiTablasAuditoria, type AuditoriaFiltros } from '@/lib/api/auditoria'
...
const accionTone = (a: Auditoria['accion']): 'success' | 'warning' | 'danger' => {
...
export default function AuditoriaPage() {
  ...
  const [filtros, setFiltros] = useState<AuditoriaFiltros>({ page: 0, size: 20 })
  ...
  const columnas: Columna<Auditoria>[] = [
    {
      ...
      render: (v) => <span className="tabular-nums">{formatoFechaHora(v.creadoEn)}</span>,
    },
    {
      key: 'usuario',
      header: t('seguridad.auditoria.columnas.usuario'),
      render: (v) => (v.usuario ? <span className="font-medium">{v.usuario}</span> : <span className="text-muted">—</span>),
    },
    {
      key: 'esquema',
    ...
    <div className="space-y-4">
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-semibold text-ink">
          <History className="h-6 w-6 text-primary" /> {t('seguridad.auditoria.titulo')}
        </h1>
        <p className="text-sm text-muted">{t('seguridad.auditoria.subtitulo')}</p>
      </div>
      ...
      <Card>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <Select
            ...
            onChange={(e) => setFiltros((f) => ({ ...f, esquema: e.target.value || undefined, page: 0 }))}
          >
            <option value="">{t('seguridad.auditoria.filtros.todos')}</option>
            {tablas.data
              ? Array.from(new Set(tablas.data.map((tt) => tt.esquema))).map((s) => (
          ...
          <Select
            ...
            value={filtros.tabla ?? ''}
            onChange={(e) => setFiltros((f) => ({ ...f, tabla: e.target.value || undefined, page: 0 }))}
          >
            <option value="">{t('seguridad.auditoria.filtros.todas')}</option>
            {(tablas.data ?? [])
              .filter((tt: AuditoriaTabla) => !filtros.esquema || tt.esquema === filtros.esquema)
              .map((tt) => (
          ...
          <Select
            ...
            onChange={(e) => setFiltros((f) => ({ ...f, accion: e.target.value || undefined, page: 0 }))}
          >
            <option value="">{t('seguridad.auditoria.filtros.todasAcciones')}</option>
            {ACCIONES.map((a) => (
          ...
          <Input
            ...
            onChange={(e) => setFiltros((f) => ({ ...f, usuario: e.target.value, page: 0 }))}
          />
          <Input
            ...
            onChange={(e) =>
              setFiltros((f) => ({ ...f, registroId: e.target.value === '' ? undefined : Number(e.target.value), page: 0 }))
            }
          />
          <Input
            ...
            value={filtros.texto ?? ''}
            onChange={(e) => setFiltros((f) => ({ ...f, texto: e.target.value, page: 0 }))}
          />

          <div className="sm:col-span-2">
            <span className="text-xs font-medium text-muted">{t('seguridad.auditoria.filtros.rango')}</span>
            <div className="mt-1">
...
function DetalleCambios({ a, t }: { a: Auditoria; t: ReturnType<typeof useT> }) {
  ...
  return (
    <div className="text-xs">
      ...
      >
        ...
        <Search className="h-3 w-3" /> {abierto ? t('seguridad.auditoria.botones.ocultar') : t('seguridad.auditoria.botones.ver')}
      </button>
      {abierto && (
        <div className="mt-2 max-w-md space-y-2">
          <div>
            <div className="text-[10px] uppercase text-muted">{t('seguridad.auditoria.campos.anterior')}</div>
            <pre className="overflow-x-auto rounded bg-slate-900 px-2 py-1 text-[11px] text-slate-100">
              {a.datosAnteriores ?? '—'}
            </pre>
          </div>
          <div>
            <div className="text-[10px] uppercase text-muted">{t('seguridad.auditoria.campos.nuevo')}</div>
            <pre className="overflow-x-auto rounded bg-slate-900 px-2 py-1 text-[11px] text-slate-100">
              {a.datosNuevos ?? '—'}
            </pre>
          </div>
