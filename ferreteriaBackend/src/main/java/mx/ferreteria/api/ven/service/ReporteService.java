...
public class ReporteService {
     ...
     * que requiere constructor sin argumentos + setters, {@link DataClassRowMapper}
     * usa el constructor
     * canónico y resuelve los nombres de columna snake_case a camelCase
     * automáticamente.
     */
    ...
    public List<ReportDtos.ProductosSinMovimientoResponse> productosSinMovimiento() {
        return jdbc.query(
                "SELECT * FROM inv.vw_productos_sin_movimiento order by dias_sin_vender desc",
                mapper(ReportDtos.ProductosSinMovimientoResponse.class));
    }

    public List<ReportDtos.MejoresCategoriasResponse> mejoresCategorias(LocalDate inicio, LocalDate fin) {
        return jdbc.query(
                """
                        SELECT ?::date AS mes, c.categoria_id, c.nombre AS categoria,
                               SUM(d.cantidad)::numeric(14,2)                       AS unidades_vendidas,
                               SUM(d.total_linea)::numeric(14,2)                    AS ingreso,
                               (SUM(d.total_linea) - SUM(d.cantidad * d.costo_unitario))::numeric(14,2)
                                                                                   AS utilidad,
                               RANK() OVER (ORDER BY SUM(d.total_linea) DESC)       AS ranking_mes,
                               RANK() OVER (ORDER BY SUM(d.cantidad) DESC)          AS ranking_historico
                        FROM ven.venta_detalles d
                        JOIN ven.ventas v ON v.venta_id = d.venta_id AND v.estado = 'COMPLETADA'
                        JOIN inv.productos p ON p.producto_id = d.producto_id
                        LEFT JOIN cat.categorias c ON c.categoria_id = p.categoria_id
                        ...
                        LIMIT 20
                        """,
                mapper(ReportDtos.MejoresCategoriasResponse.class),
                inicio, inicio, fin);
    }
}